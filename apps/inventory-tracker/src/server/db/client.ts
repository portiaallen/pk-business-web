...
function createPrismaClient(): PrismaClient {
  ...
  // PostgreSQL requires schema provider = postgresql (not yet migrated).
  // Use Turso (libsql://) for production until Postgres schema migration is complete.
  if (
    databaseUrl.startsWith("postgres://") ||
    databaseUrl.startsWith("postgresql://")
  ) {
    throw new Error(
      "PostgreSQL support requires a schema migration. Use Turso (libsql://) for now."
    );
  }
